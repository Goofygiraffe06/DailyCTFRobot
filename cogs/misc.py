...
class misc(commands.Cog):
    ...
    async def _ping(self, ctx):
        ...
        gateway_latency = (t.created_at - ctx.message.created_at).total_seconds() * 1000 # Convert to milliseconds
        await t.delete() # Delete the temporary message

        # Create an embed
        embed = discord.Embed(title="Ping Information", color=0x00ff00)
        embed.add_field(name="Server Latency", value=f"{server_latency} ms", inline=False)
        embed.add_field(name="Gateway Latency", value=f"{gateway_latency} ms", inline=False)
