...
def load_challenge_data():
    try:
        ...
        logging.warning("Challenge data not found or corrupted. Returning empty data.")
        return {}
...
def load_config():
    try:
        ...
        logging.warning("Config not found or corrupted. Returning empty data.")
        return {}
...
def save_config(config_data):
    try:
        ...
        return config_data  # Return the saved data
    except Exception as e:
...
async def end_challenge(bot):
    ...
    if challenge_channel:
        await challenge_channel.send(f"Day-{challenge_data.get('day', 'N/A')} Challenge has finished!")
        logging.info(f"Day-{challenge_data.get('day', 'N/A')} challenge has been finished...")

        # Check if a write-up exists and then send the appropriate message
        if 'writeup' in challenge_data and challenge_data['writeup']:
            await challenge_channel.send(f"Writeup for Day-{challenge_data.get('day', 'N/A')}: {challenge_data['writeup']}")
        else:
            await challenge_channel.send(f"No writeup provided for Day-{challenge_data.get('day', 'N/A')}.")
...
class RateButton(discord.ui.Button):
  ...
  async def callback(self, interaction: discord.Interaction):
      ...
      # Append the user's rating to the challenge data
...
async def release_hints(bot):
  ...
  if (
      ...
      challenge_channel = bot.get_channel(int(config.get("channel_id", 0)))
      if challenge_channel:
          await challenge_channel.send(f"Hint for Day-{challenge_data.get('day', 'N/A')}: `{challenge_data.get('hints', 'No hints available')}`")
          logging.info(f"Hint for Day-{challenge_data.get('day', 'N/A')} released.")

      challenge_data["hints_revealed"] = True
      save_challenge_data(challenge_data)
  else:
      logging.warning("Hints were either already revealed or there is an active leaderboard. No hint was released.")
